...
router.get('/', validatePagination, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if ("page" in req.query) {
        ...
        const users = yield database.all('SELECT * FROM users LIMIT ? OFFSET ?', [rowsPerPage, (page - 1) * rowsPerPage]);
...
router.get('/:id', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const database = yield database_1.default;
    const user = yield database.get('SELECT * FROM users WHERE id = ?', [req.params.id]);
    return res.json(user);
}));
router.post('/', validate, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const database = yield database_1.default;
        yield database.run('INSERT INTO users (username, password) VALUES (?, ?)', [req.body.username, req.body.password]);
